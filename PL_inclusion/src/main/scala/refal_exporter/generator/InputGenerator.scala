...
package refal_exporter.generator

import refal_exporter.Grammar.DomainTag._
import refal_exporter.compiler.CompilerData
import refal_exporter.lexer.{ConstToken, IdentToken, SignToken, VariableToken}
import refal_exporter.parser.Tree._
...
class InputGenerator(val compiler: CompilerData, val ast: Node) {
  ...
  private val getTasksString: List[Node] => List[String] = patterns => {
    ...
    val getVarString: (mutable.Map[Int, Int], Int, String) => String = (m, limit, varType) =>
      m.filter(pair => pair._1 < limit).values.map(index => s"$varType$index").toList.sorted.mkString("{", ",", "}")
    ...
    def patternToString(sb: mutable.StringBuilder,
      ...
      remain match {
        ...
        case head :: tail =>
          head match {
            case _: Nonterm =>
              patternToString(sb, shiftE, shiftT, getChildren(head):::tail)
            case term: Term =>
              term.token match {
